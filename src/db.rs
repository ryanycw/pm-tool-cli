...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        ...
        let new_epic_id = db_state.last_item_id;
        db_state.epics.insert(new_epic_id, epic);
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        let mut db_state = self.read_db()?;
        db_state.last_item_id += 1;
        let new_story_id = db_state.last_item_id;
        db_state.stories.insert(new_story_id, story);
        let epic = db_state
            .epics
            .get_mut(&epic_id)
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        ...
        let db_state: DBState = serde_json::from_str(&file)?;
        Ok(db_state)
    }
