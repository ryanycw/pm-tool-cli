    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::CreateEpic => {
                ...
                self.pages.push(Box::new(EpicDetail {
                    epic_id: self
                        .db
                        .read_db()
                        .with_context(|| "Failed to read database")?
                        .last_item_id,
                    db: Rc::clone(&self.db),
                }));
            }
            Action::UpdateEpicStatus { epic_id } => {
                ...
                println!("update epic status: {:?}", status);
                self.db.update_epic_status(
                    epic_id,
                    status.with_context(|| "Failed to update epic status")?,
                )?;
            }
            Action::DeleteEpic { epic_id } => {
                let deleted = (self.prompts.delete_epic)();
                if deleted {
            ...
            Action::UpdateStoryStatus { story_id } => {
                ...
                self.db.update_story_status(
                    story_id,
                    status.with_context(|| "Failed to update story status")?,
                )?;
            }
            Action::DeleteStory { epic_id, story_id } => {
                let deleted = (self.prompts.delete_story)();
                if deleted {
...
mod tests {
    ...
    fn should_start_on_home_page() {
        ...
        let current_page: &Box<dyn Page> = nav.get_current_page().unwrap();
